...
export async function fetchLinkedInPosts({
  ...
  apiVersion = process.env.LINKEDIN_API_VERSION || "202605",
  count = 3
} = {}) {
  if (!accessToken) {
  ...
  const resolvedOrganizationId =
    organizationId || (await resolveOrganizationId({ accessToken, apiVersion, vanityName }));
  ...
  const data = await requestLinkedIn(postsUrl, {
    ...linkedInHeaders(accessToken, apiVersion),
